...
const £      = n  => "£" + Math.abs(+n || 0).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  ...
  return (
    <div style={{ background:"var(--surface)", borderRadius:12, padding:14, border:`0.5px solid ${CATS[cat].color}33` }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        ...
        <div>
          <span style={{ fontSize:14, fontFamily:"'JetBrains Mono',monospace", fontWeight:500 }}>{£(tot)}</span>
          {diff!==null&&diff!==0&&<span style={{ fontSize:11, marginLeft:6, color:diff>0?"var(--red)":"var(--green)" }}>{diff>0?"+":""}{£(diff)}</span>}
        </div>
      </div>
...
function Dashboard({ md, months, data, savings, setSavCur }) {
  ...
  const DTip=({active,payload})=>{ if(!active||!payload?.length) return null; const d=payload[0].payload; return <div style={{ background:"var(--bg)", border:"0.5px solid var(--border-mid)", borderRadius:8, padding:"8px 12px", fontSize:12 }}><div style={{ color:d.color, fontWeight:500, marginBottom:2 }}>{d.name}</div><div>{£(d.value)} · {pct(d.value,total)}%</div></div>; };
  const CTip=({active,payload,label})=>{ if(!active||!payload?.length) return null; return <div style={{ background:"var(--bg)", border:"0.5px solid var(--border-mid)", borderRadius:8, padding:"8px 12px", fontSize:12 }}><div style={{ fontWeight:500, marginBottom:6 }}>{label}</div>{payload.map(p=><div key={p.name} style={{ display:"flex", gap:8, alignItems:"center", marginBottom:2 }}><div style={{ width:8, height:8, borderRadius:2, background:p.color }} /><span style={{ color:"var(--text-muted)" }}>{p.name}:</span><span style={{ fontFamily:"'JetBrains Mono',monospace" }}>{metric==="savings"?`${p.value}%`:£(p.value)}</span></div>)}</div>; };

  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        <StatCard label="Combined income" val={£(income)} />
        <StatCard label="Total spend"     val={£(total)}  sub={`${pct(total,income)}% of income`} />
        <StatCard label="Monthly savings" val={£(savAmt)} color="var(--green)" />
        <StatCard label="Left over"       val={£(left)}   color={left<0?"var(--red)":"var(--green)"} big />
      </div>
      ...
      <div style={{ background:"var(--surface)", borderRadius:8, padding:"16px 20px", marginBottom:20 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", marginTop:6, fontSize:11, color:"var(--text-muted)" }}><span>{£(savings.current)} saved</span><span>{£(savings.target-savings.current)} to go</span></div>
      </div>
    ...
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:14, flexWrap:"wrap" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ fontSize:12, color:"var(--text-muted)" }}>Spend: <span style={{ fontFamily:"'JetBrains Mono',monospace", color:st>si?"var(--red)":"var(--text)" }}>{£(st)}</span></span>
        </div>
      </div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"220px 1fr", gap:20 }}>
      ...
      {!cs?<div style={{ display:"flex", alignItems:"center", justifyContent:"center", color:"var(--text-faint)", fontSize:13 }}>Select or create a scenario to get started</div>:(
        <div>
          ...
          {view==="compare"&&<div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:20 }}><StatCard label="Base total spend" val={£(tBase)} /><StatCard label="Scenario total" val={£(tScen)} /><StatCard label="Difference" val={`${tScen-tBase>=0?"+":""}${£(tScen-tBase)}`} color={tScen>tBase?"var(--red)":"var(--green)"} /></div>
            <div style={{ background:"var(--surface)", borderRadius:8, overflow:"hidden" }}><table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}><thead><tr style={{ borderBottom:"0.5px solid var(--border)" }}>{["Section · Category","Base","Scenario","Change"].map((h,i)=><th key={h} style={{ padding:"10px 14px", textAlign:i===0?"left":"right", fontWeight:500, fontSize:11, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:"0.05em" }}>{h}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={i} style={{ borderBottom:"0.5px solid var(--border)" }}><td style={{ padding:"8px 14px" }}><span style={{ color:CATS[r.c].color, fontWeight:500, marginRight:4 }}>●</span><span style={{ color:"var(--text-muted)", marginRight:4 }}>{SL[r.s]}</span>{CATS[r.c].label}</td><td style={{ padding:"8px 14px", textAlign:"right", fontFamily:"'JetBrains Mono',monospace" }}>{£(r.base)}</td><td style={{ padding:"8px 14px", textAlign:"right", fontFamily:"'JetBrains Mono',monospace" }}>{£(r.scen)}</td><td style={{ padding:"8px 14px", textAlign:"right", fontFamily:"'JetBrains Mono',monospace", color:r.diff>0?"var(--red)":r.diff<0?"var(--green)":"var(--text-faint)" }}>{r.diff===0?"—":`${r.diff>0?"+":""}${£(r.diff)}`}</td></tr>)}<tr style={{ borderTop:"0.5px solid var(--border-mid)" }}><td style={{ padding:"10px 14px", fontWeight:500 }}>Total</td><td style={{ padding:"10px 14px", textAlign:"right", fontFamily:"'JetBrains Mono',monospace", fontWeight:500 }}>{£(tBase)}</td><td style={{ padding:"10px 14px", textAlign:"right", fontFamily:"'JetBrains Mono',monospace", fontWeight:500 }}>{£(tScen)}</td><td style={{ padding:"10px 14px", textAlign:"right", fontFamily:"'JetBrains Mono',monospace", fontWeight:500, color:tScen>tBase?"var(--red)":"var(--green)" }}>{tScen===tBase?"—":`${tScen>tBase?"+":""}${£(tScen-tBase)}`}</td></tr></tbody></table></div>
          </div>}
        </div>
      )}
    </div>
